...
function ShowInputBox(title, desc, allowClose, controls) {
    ...
    return new Promise((resolve) => {
        ...
        const overlay = document.getElementById('input-box-overlay');
        ...
        controls.forEach(control => {
            switch (control.type) {
                ...
                case 'number':
                    ...
                    break;
                case 'select':
                    let options = control.options.map(option => `<option value="${option}">${option}</option>`).join('');
                    inputBoxControls.innerHTML += `
                                <label for="${control.id}">${getNpcText(control.label)}</label>
                ...
                case 'table':
                    ...
                    let rows = control.data.map(row => row[1] === undefined || (Array.isArray(row[1]) && row[1].length === 0) ? '' : `
                                <tr>
                                    <td>${row[0]}</td>
                                    ${getTd(row[1])}
                                </tr>
                ...
                case 'list':
                    ...
                    let listItems = control.options.map(option => `
                ...
                case 'switch':
                    ...
                    let switchItems = control.options.map(option => `
                    ...
                    break;
                case 'group':
                    let groupContent = control.controls.map(subControl => {
                        return `<div>${renderControl(subControl)}</div>`;
                    }).join('');
                    inputBoxControls.innerHTML += `
...
function toggleListSelection(listId, option, maxSelect) {
    ...
    if (button.classList.contains('selected')) {
        ...
        if (selectedButtons.length < maxSelect) {
